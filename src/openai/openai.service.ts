    ...
    async updateDialogue(dialogId: string, message: string, role: DialogRole): Promise<any> {
        ...
        if (!this.dialogues[dialogId]) {
            ...
            const prompt = 'Ти — чат-бот служби підтримки ТАСКОМБАНКУ. Твоя задача — надавати інформацію та допомогу згідно з базою знань "ВІДПОВІДІ НА ЧАСТІ ЗАПИТАННЯ",' +
                ...
                'ти повинен запропонувати звернутися до Контакт-центру за номерами 0 800 503 580 або +38 044 393 25 90. Твої відповіді мають бути вичерпними та зрозумілими,' +
                ' з використанням ємодзі для кращої виразності у форматі телеграм-повідомлень. Важливо завжди звертатися до клієнтів на "Ви",' +
                ' підтримуючи ввічливий та професійний тон комунікації. Відповідай українською мовою,' +
